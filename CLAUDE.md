...
- `session_runner.py` — **Main entry point.** Single long-lived script that starts the tutor, generates student responses, captures tutor responses, and writes JSONL. Supports LLM-driven (`--student-llm openai`) or deterministic (canned) student responses. 17 built-in scenarios across 8 domains.
